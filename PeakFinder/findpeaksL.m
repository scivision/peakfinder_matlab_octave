function P=findpeaksL(x,y,SlopeThreshold,AmpThreshold,smoothwidth,peakgroup,smoothtype)
...
% number and position, height, width, and area of each peak, assuming a
% Lorentzian peak shape. Arguments "slopeThreshold", "ampThreshold" and
...
% See http://terpconnect.umd.edu/~toh/spectrum/Smoothing.html and 
% http://terpconnect.umd.edu/~toh/spectrum/PeakFindingandMeasurement.htm
% T. C. O'Haver, 1995.  Version 1.1, Last revised December, 2014
% line 61: changed log(abs(yy)) to rmnan(log(yy)); added rmnan function
% 
% Example: 
% Three noisy Lorentzian peaks at x=20,50,80, all heights=1.0; all widths=10.
%   x=1:.2:100;
%   y=lorentzian(x,20,10)+lorentzian(x,50,10)+lorentzian(x,80,10)+.01.*randn(size(x));
%   findpeaksL(x,y,0.0004,0.3,17,17,3)
% Compare to findpeaks(x,y,0.0004,0.3,17,17,3) with the same signal.
% 
% Related functions:
% findvalleys.m, findpeaks.m, findpeaksb.m, findpeaksplot.m, peakstats.m,
        ...
        if d(j)-d(j+1) > SlopeThreshold*y(j), % if slope of derivative is larger than SlopeThreshold
            if y(j) > AmpTest,  % if height of peak is larger than AmpThreshold
                ...
                if isnan(PeakX) || isnan(PeakY) || PeakY<AmpThreshold,
                    ...
                    P(peak,:) = [round(peak) PeakX PeakY MeasuredWidth  1.57.*PeakY*MeasuredWidth];
