function P=findpeaksplot(x,y,SlopeThreshold,AmpThreshold,smoothwidth,peakgroup,smoothtype)
% function
% P=findpeaksplot(x,y,SlopeThreshold,AmpThreshold,smoothwidth,peakgroup,...
% smoothtype) Function to locate and plot the positive peaks in a noisy x-y
% time series data set. Detects peaks by looking for downward
...
% T. C. O'Haver, 1995.  Version 6.1, Last revised December, 2014
% line 61: changed log(abs(yy)) to rmnan(log(yy)); added rmnan function
% Examples:
% findpeaksplot(0:.01:2,humps(0:.01:2),0,-1,5,5)
% x=[0:.01:50];findpeaksplot(x,cos(x),0,-1,5,5)
% x=[0:.01:5]';findpeaksplot(x,x.*sin(x.^2).^2,0,-1,5,5)
%
% Related functions:
% findvalleys.m, findpeaksL.m, findpeaksb.m, findpeaks.m, peakstats.m,
